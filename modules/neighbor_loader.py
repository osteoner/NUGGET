...
class LastNeighborLoader:
    def __init__(self, num_nodes: int, size: int, device=None, filter_invalid_neighbors : bool = True):
        self.size = size
        ...
        self.filter_invalid_neighbors = filter_invalid_neighbors
        self.reset_state()
    ...
    def __call__(self, n_id: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        if self.filter_invalid_neighbors:
            mask = e_id >= 0
        else:
            mask = torch.ones_like(e_id).bool()
...
class LastNeighborLoaderTGAT(LastNeighborLoader):
    def __init__(self, num_nodes: int, num_edges : int, size: int, device=None, filter_invalid_neighbors : bool = True):
        super().__init__(num_nodes+1, size, device, filter_invalid_neighbors)
    ...
    def __call__(self, n_id: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        mask = e_id >= 0
        neighbs_out = (torch.ones((n_id.size(0), self.size), device=n_id.device)*self.num_nodes).long()
        neighbs_out[mask] = neighbors[mask]
        eid_batch = (torch.ones((n_id.size(0), self.size), device=n_id.device)*self.num_edges).long()
        eid_batch[mask] = e_id[mask]

        return neighbs_out, eid_batch
        ...
        self._assoc[n_id] = torch.arange(n_id.size(0), device=n_id.device)
        neighbors, nodes = self._assoc[neighbors], self._assoc[nodes]
        edge_idx = torch.stack([neighbors, nodes])
        neighbs_out = (torch.ones((n_id.size(0), self.size), device=n_id.device)*self.num_nodes).long()
        eid_batch = torch.zeros((n_id.size(0), self.size), device=n_id.device).long()
