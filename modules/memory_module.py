...
# Ensure TimeEncoder is imported if this is a separate file
# from modules.emb_module import TimeEncoder 
...
class TGNMemory(torch.nn.Module):
    ...
    def __init__(
        ...
        # last_update is time (float)
        last_update = torch.empty(self.num_nodes, dtype=torch.float).to(self.device)
        self.register_buffer("last_update", last_update)
        
        # _assoc stores indices, so it MUST be Long
        self.register_buffer("_assoc", torch.empty(num_nodes, dtype=torch.long).to(self.device))
    ...
    def _reset_message_store(self):
        # Indices (src, dst) must be Long
        i_long = self.memory.new_empty((0,), device=self.device, dtype=torch.long)
        # Time (t) must be Float
        i_float = self.memory.new_empty((0,), device=self.device, dtype=torch.float)
        # Message content is Float
        msg = self.memory.new_empty((0, self.raw_msg_dim), device=self.device, dtype=torch.float)

        self.msg_s_store = {j: (i_long, i_long, i_float, msg) for j in range(self.num_nodes)}
        self.msg_d_store = {j: (i_long, i_long, i_float, msg) for j in range(self.num_nodes)}
    ...
    def _get_updated_memory(self, n_id: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # === FIX IS HERE: pass dim_size as a KEYWORD argument ===
        last_update = scatter(t, idx, dim=0, dim_size=dim_size, reduce="max")[n_id]
    ...
    def _compute_msg(
        self, n_id: Tensor, msg_store: Dict, msg_module: Callable
    ):
        data = [msg_store[i] for i in n_id.tolist()]
        src, dst, t, raw_msg = list(zip(*data))
        
        src = torch.cat([x.to(self.device) for x in src], dim=0).long().to(self.device)
        dst = torch.cat([x.to(self.device) for x in dst], dim=0).long().to(self.device)
        t = torch.cat([x.to(self.device) for x in t], dim=0).float().to(self.device)
        raw_msg = torch.cat([x.to(self.device) for x in raw_msg], dim=0).float().to(self.device)
...
class DyRepMemory(torch.nn.Module):
    ...
    def _reset_message_store(self):
        i = self.memory.new_empty((0, ), device=self.device, dtype=torch.float)
    ...
    def _compute_msg(self, n_id: Tensor, msg_store: TGNMessageStoreType, msg_module: Callable, 
        ...
        t_rel = t - self.last_update[src.long()]
        t_enc = self.time_enc(t_rel.to(raw_msg.dtype))
