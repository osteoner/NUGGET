...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat, edge_times, num_nodes : int,
                 attn_mode='prod', use_time='time', agg_method='attn', node_dim=None, time_dim=None,
                 num_layers=3, n_head=4, null_idx=0, num_heads=1, drop_out=0.1, seq_len=None, num_neighbors : int = 20):
        super(TGAN, self).__init__()
        
        self.num_layers = num_layers 
        self.ngh_finder = ngh_finder
        self.null_idx = null_idx
        self.edge_times = torch.cat((edge_times, torch.zeros(1).to(edge_times.device)))
        self.logger = logging.getLogger(__name__)
        
        
        # Concatenate node features with a zero vector for padding
        self.n_feat_th = torch.cat([n_feat, torch.zeros(1, n_feat.shape[1]).to(n_feat.device)], dim=0)
        self.e_feat_th = torch.cat([e_feat, torch.zeros(1, e_feat.shape[1]).to(e_feat.device)], dim=0)
        self.n_feat_th = torch.nn.Parameter(self.n_feat_th, requires_grad=False)
        self.e_feat_th = torch.nn.Parameter(self.e_feat_th, requires_grad=False)

        self.edge_raw_embed = torch.nn.Embedding.from_pretrained(self.e_feat_th, freeze=True, padding_idx=len(edge_times))
        self.node_raw_embed = torch.nn.Embedding.from_pretrained(self.n_feat_th, freeze=True, padding_idx=num_nodes)
        ...
        if agg_method == 'attn':
            ...
            self.attn_model_list = torch.nn.ModuleList([MultiHeadAttention(node_feat_dim=self.n_feat_dim, 
                                                               edge_feat_dim=self.e_feat_dim, 
                                                               time_feat_dim=self.feat_dim,
                                                               num_heads=n_head, 
                                                               dropout=drop_out) for _ in range(num_layers)])
            self.merge_layers = torch.nn.ModuleList([MergeLayer(self.feat_dim*num_heads if num_heads != 1 else 2*self.feat_dim, self.feat_dim, self.feat_dim, self.feat_dim) for _ in range(num_layers)])
    ...
    def forward(self, src_idx_l, cut_time_l, target_idx_l = None):
        
        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers)
        if target_idx_l is not None:
    ...
    def contrast(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l):
        ...
        return src_embed, target_embed, background_embed
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers):
        ...
        if curr_layers == 0:
            ...
            src_ngh_node_batch, src_ngh_eidx_batch = self.ngh_finder(src_idx_l)
            src_ngh_t_batch = self.edge_times[src_ngh_eidx_batch]

            src_ngh_node_batch_th = src_ngh_node_batch.long().to(device)
            src_ngh_eidx_batch = src_ngh_eidx_batch.long().to(device)
            
            # src_ngh_t_batch_delta = cut_time_l[:, np.newaxis] - src_ngh_t_batch
            # Translate above line to torch tensor
            
            src_ngh_t_batch_delta = cut_time_l_th[:, ...] - src_ngh_t_batch
            src_ngh_t_batch_th = (src_ngh_t_batch_delta).float().to(device)
            
            # get previous layer's node features
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv(src_ngh_node_batch_flat, src_ngh_t_batch_flat, curr_layers=curr_layers - 1)
            ...
            mask = src_ngh_node_batch_th == self.num_nodes
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                   ...
                                   src_ngn_edge_feat, 
                                   mask)
            local = self.merge_layers[curr_layers - 1](local, src_node_conv_feat)
            return local